...
async function checkRepositorySettings() {
  ...
  try {
    ...
    const repoData = exec(
      'gh api repos/pantheon-org/opencode-font --jq "{allow_squash_merge, allow_merge_commit, allow_rebase_merge, delete_branch_on_merge}"',
    );
    ...
    if (isOptimal) {
      ...
      console.log('   Go to: https://github.com/pantheon-org/opencode-font/settings');
...
async function checkBranchProtection() {
  ...
  try {
    const protectionData = exec(
      'gh api repos/pantheon-org/opencode-font/branches/main/protection 2>/dev/null',
    );
